...
Ending::Ending()
{
	...
	// Button
	g_end_button_rect.x = 250;
	g_end_button_rect.y = 350;
	g_end_button_rect.w = 200;
	g_end_button_rect.h = 100;
	...
	SDL_Surface* tmp_surface_1 = TTF_RenderUTF8_Blended(g_font_intro, CW2A(L"Retry?", CP_UTF8), black);
	//텍스트 가져오기
...
void Ending::Render()
{
	...
	SDL_RenderFillRect(g_renderer, &g_end_button_rect);
	...
	if (SDL_PollEvent(&event))
	{
		...
		case SDL_MOUSEBUTTONDOWN:
			...
			if (event.button.button == SDL_BUTTON_LEFT)
			{
				...
				if (mouse_x > g_end_button_rect.x &&
					mouse_y > g_end_button_rect.y &&
					mouse_x < g_end_button_rect.x + g_end_button_rect.w &&
					mouse_y < g_end_button_rect.y + g_end_button_rect.h
					)
				{					
